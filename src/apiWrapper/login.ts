import { SessionConfig, setCookie, updateSession } from "vinxi/http";
...
export async function discGolfMetrixLogin(email: string, password: string) {
  ...
  const maxAgeNum = parseInt(rhmMaxAge.split("=")[1]);
  if (isNaN(maxAgeNum)) throw new Error("RHM Max-Age not a number");
